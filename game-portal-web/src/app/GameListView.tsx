      ...
      {gameList.map((game, index) => {
        return (
          <li
            ...
            className={`${styles.gameEle} ${game.isAvailable ? "" : styles.disable} ${index === curSelect ? styles.select : ""}`}
          >
            {game.isAvailable ? (
              <Link className={styles.gameLink} href={`${game.link}${token}${!token || token === "null" ? "&isLocal=true" : ""}`}>
                <span className={styles.gameName}>{game.name}</span>
              </Link>
            ) : (
              <div className={styles.gameLink}>
