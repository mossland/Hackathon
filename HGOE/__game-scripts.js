...
DummyServer.prototype.betGame = function(isOdd) {
    let deck = getDeck();
    deck = shuffle(deck);
    this.hiddenCard = deck[0];

    let resultIsOdd = (this.openCard.number + this.hiddenCard.number) % 2 === 1;

    

    let isWin = (resultIsOdd === isOdd);
    let profit = 0;
    if (isWin){
    ...
    console.log(this.openCard.number, this.hiddenCard.number, isOdd, resultIsOdd, isWin, profit);
...
CardFront.prototype.setCardType = function(cardType) {
    ...
    this.numberImage.forEach(e => e.element.texture = target);
};
...
Middle.prototype.setResultText = function(sumNumber) {
    AudioController.instance.playSound('Open');
    let isEven = (sumNumber % 2) === 0 ? true : false;

    this.setResultText2(isEven);
};

Middle.prototype.setResultText2 = function(isEven) {
    this.resultText.element.text = isEven ? 'EVEN' : 'ODD';

    let parent = this.resultText.getParent();
    parent.element.color = isEven ? getEColor() : getOColor();
};
...
Card.prototype.setCard = function(cardType, number) {
    ...
    this.cardTypeText.element.text = isEven ? 'EVEN' : 'ODD';
    this.cardFront.script.cardFront.setCard(cardType, number);  

    let parent = this.cardTypeText.getParent();
    parent.element.color = isEven ? getEColor() : getOColor();
};
...
GameController.prototype.startGame = function(betAmount) {
    ...
    setTimeout(() => { 
        Middle.instance.setOpenCard(gameInfo.openCard.type, gameInfo.openCard.number);
        Bottom.instance.setStartGame();
    }, 1000);
};

GameController.prototype.betGame = async function(isOdd) {
    let result = DummyServer.instance.betGame(isOdd);

    await delay(1000);
    Middle.instance.setHiddenCard(result.hiddenCard.type, result.hiddenCard.number);

    await delay(700);
    let sum = result.hiddenCard.number + result.openCard.number;
    Middle.instance.setResultText(sum);
...
Bottom.attributes.add('oddButton', {type: 'entity'});
Bottom.attributes.add('evenButton', {type: 'entity'});
...
Bottom.prototype.initialize = function() {
    ...
    this.setButton(this.oddButton, this.onClickOdd);
    this.setButton(this.evenButton, this.onClickEven);
...
Bottom.prototype.onClickOdd = function(){
    this.disableAll();
    this.changeTexture(this.oddButton, this.odd_active);
    
    AudioController.instance.playSound('Click');
    GameController.instance.betGame(true);
};

Bottom.prototype.onClickEven = function(){
    this.disableAll();
    this.changeTexture(this.evenButton, this.even_active);

    AudioController.instance.playSound('Click');
    GameController.instance.betGame(false);
};
...
Bottom.prototype.disableAll = function() {
    ...
    this.changeButtonState(this.oddButton, false);
    this.changeButtonState(this.evenButton, false);
...
Bottom.prototype.setIdle = function() {
    ...
    setTimeout( () => {
        ...
        this.changeTexture(this.oddButton, this.odd_inactive);
        this.changeTexture(this.evenButton, this.even_inactive);
    }, 1000);
};
...
Bottom.prototype.setStartGame = function() {
    ...
    setTimeout( () => {
        this.changeButtonState(this.oddButton, true);
        this.changeButtonState(this.evenButton, true);
    }, 1000);
};
