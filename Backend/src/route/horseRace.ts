...
import { validateHorseRaceGameInput, createGameStateValidator, validateBetAmount } from '../middleware/validator';
...
const ODDS_BY_COUNT = [2, 3, 5, 8, 15, 40] as const;
const WIN_THRESHOLD = 8 as const;

function shuffleArrayWithHashKeepTop(array: Card[], topCards: Card[], hash: string): Card[] {
  const pool = array.slice();
  const keptTop: Card[] = [];

  for (const t of topCards) {
    const idx = pool.findIndex(c => c.type === t.type && c.number === t.number);
    if (idx !== -1) {
      keptTop.push(t);
      pool.splice(idx, 1);
    }
...
function runRace(shuffledDeck: Card[], topCardsCount: number, threshold = WIN_THRESHOLD) {
...
router.post('/result', createGameStateValidator(horseRaceGameId), validateBetAmount, validateHorseRaceGameInput, verifyToken, async (req, res, next) => {
	try {
    ...
    const topCards: Card[] = req.body.topCards as Card[];
        ...
        const shuffledDeck = shuffleArrayWithHashKeepTop(deck, topCards, hash);
