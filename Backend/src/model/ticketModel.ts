...
export default class TicketModel {
		...
		return new Promise((resolve, reject) => {
			db.transaction(async (trx) => {
				try {
					...
					const revealCardIdx = new Big(revealCardDecimal).div(new Big(2).pow(N_BIT)).mul(new Big(deck.length).pow(4)).round(0, 0).mod(deck.length);
					...
					const hiddenCardIdx = new Big(hiddenCardDecimal).div(new Big(2).pow(N_BIT)).mul(new Big(deck.length).pow(4)).round(0, 0).mod(deck.length);
	...
	public static async spendHgTicket(userId: string, userPickNum: number) {
		return new Promise((resolve, reject) => {
			db.transaction(async (trx) => {
				const unspendTicketList = await trx.select('*')
					.from('ticket')
					.innerJoin(
						'user_ticket_history',
						'ticket.ticketId',
						'=',
						'user_ticket_history.ticketId',
					).where({
				...
				if (payout > 0) {
					await Platform.instance.updateUserPoint(
						userId,
						unspendTicket.ticketId,
						new Big(unspendTicket.betAmount).mul(payout).toNumber(),
					);
				}
